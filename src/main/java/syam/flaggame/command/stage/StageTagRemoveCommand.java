 ...
 * @author toyblocks
 */
public class StageTagRemoveCommand extends BaseCommand {

    public StageTagRemoveCommand(FlagGameAPI api) {
        super(
                api,
                false,
                2,
    ...
    protected void execute(List<String> args, CommandSender sender, Player player) throws FlagGameException {
        ...
        stage.removeTag(args.get(1));
        OnelineBuilder.newBuilder()
                .value(stage.getName()).info("のタグ")
                .value(args.get(1)).info("を削除しました！")
                .sendTo(sender);
    }
