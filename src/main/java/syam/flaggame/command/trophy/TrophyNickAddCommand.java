 ...
 * @author toyblocks
 */
public class TrophyNickAddCommand extends TrophyEditCommand<Trophy> {

    public TrophyNickAddCommand(FlagGameAPI api) {
        super(
                api,
                2,
                "<position> <nick> <- add selected trophy's reward nick",
    ...
    protected void execute(List<String> args, GamePlayer gamePlayer, Player player, Trophy trophy) throws CommandException {
        ...
        try {
            trophy.addRewardNick(position, nick);
        } catch (InvalidNameException ex) {
            throw new CommandException("&c'" + nick + "'は無効なニックネームです", ex);
        }
