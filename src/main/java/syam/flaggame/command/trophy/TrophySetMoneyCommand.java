 ...
 * @author toyblocks
 */
public class TrophySetMoneyCommand extends TrophyEditCommand<Trophy> {

    public TrophySetMoneyCommand(FlagGameAPI api) {
        super(
                api,
                1,
                "<amount> <- set selected trophy's reward money",
    ...
    protected void execute(List<String> args, GamePlayer gamePlayer, Player player, Trophy trophy) throws CommandException {
        ...
        trophy.setRewardMoney(amount);
        OnelineBuilder.newBuilder()
                .info("トロフィー").value(trophy.getName())
                .info("の報酬金額は").value(Actions.formatMoney(amount))
                .info("に設定されました！")
                .sendTo(player);
    }
