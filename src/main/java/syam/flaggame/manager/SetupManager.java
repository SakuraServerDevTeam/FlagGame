/**
 * FlagGame - Package: syam.flaggame.game Created: 2012/09/26 22:54:17
 */
...
public class SetupManager {
     ...
     *            対象のプレイヤー
     * @param state
     *            true = 管理モードにする/false = しない
     */
    public static void setManager(Player player, Configables conf) {
        // conf == null ならマネージモード解除
        if (conf == null) {
            if (managersMap.containsKey(player.getName())) managersMap.remove(player.getName());
        }
        // マネージモード設定
        else {
    ...
    public static void removeManager(Player player, boolean silent) {
        ...
        if (conf != null) {
            ...
            if (!silent) Actions.message(player, "&a" + conf.getConfigName() + "管理モードを解除しました！");
        }
